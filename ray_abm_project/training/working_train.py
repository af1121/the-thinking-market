#!/usr/bin/env python3
"""
Working RL training script for Ray 2.46.0
"""
...
def main():
    ...
    try:
        for i in range(num_iterations):
            ...
            if (i + 1) % 10 == 0:
                checkpoint_dir = f"checkpoints/working_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                os.makedirs(checkpoint_dir, exist_ok=True)
                checkpoint_path = algo.save(checkpoint_dir)
                print(f"💾 Checkpoint saved: {checkpoint_path}")
        
        # Save final model
        final_checkpoint_dir = f"checkpoints/working_ppo_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        checkpoint_dir = f"checkpoints/working_ppo_interrupted_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        os.makedirs(checkpoint_dir, exist_ok=True)
        checkpoint_path = algo.save(checkpoint_dir)
