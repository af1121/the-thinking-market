#!/usr/bin/env python3
"""
Final working RL training script for Ray 2.46.0
"""
...
def main():
    ...
    print("🚀 Starting RL training with Ray 2.46.0...")
    ...
    # Create PPO configuration using new API
    config = (PPOConfig()
             .environment("MarketEnv", env_config=env_config)
             .framework("torch")
             .env_runners(num_env_runners=0)  # No remote workers
             .training(
    ...
    num_iterations = 20  # Reduced for faster completion
    ...
    try:
        for i in range(num_iterations):
            ...
            # Print progress - try different metric names
            episode_reward_mean = (result.get("env_runners/episode_reward_mean", 0) or 
                                 result.get("episode_reward_mean", 0) or
                                 result.get("sampler_results/episode_reward_mean", 0))
            episode_len_mean = (result.get("env_runners/episode_len_mean", 0) or
                               result.get("episode_len_mean", 0) or
                               result.get("sampler_results/episode_len_mean", 0))
            ...
            if (i + 1) % 10 == 0:
                checkpoint_dir = os.path.abspath(f"checkpoints/final_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
                os.makedirs(checkpoint_dir, exist_ok=True)
                checkpoint_path = algo.save(checkpoint_dir)
                print(f"💾 Checkpoint saved: {checkpoint_path}")
        
        # Save final model
        final_checkpoint_dir = os.path.abspath(f"checkpoints/final_ppo_complete_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        checkpoint_dir = os.path.abspath(f"checkpoints/final_ppo_interrupted_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        os.makedirs(checkpoint_dir, exist_ok=True)
        checkpoint_path = algo.save(checkpoint_dir)
